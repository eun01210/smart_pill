/*
  ...
  time 라이브러리를 사용함으로써 해결할 수 있으며, 후에 바꿀 것으로 계획함.
 */
...
unsigned long t = 0; // 아두이노가 켜진 후 부터의 시간을 담기 위한 변수 (대략 47일?)
int stat = false; // 릴레이의 상태를 담아두기 위한 변수
...
unsigned long pt = 0; // 시간 간격을 두고 물을 분사하기 위한 변수이지만, 현재는 사용하지 않았음
unsigned long ptt = 0; // 물을 특정 시간동안 분사하기 위한 변수이지만, 현재는 주석처리 되있음
int pum = true; // 모터 드라이버가 작동하는지 확인하기 위한 변수
long ltime = 0; // LED를 작동시킬 시간을 설정하는 변수
int lser = true; // 릴레이가 항상 작동하는 경우와 반대의 경우를 설정하기 위한 변수
long hour = 3600000; // 밀리초를 시간으로 환산하는 변수
...
int ppap = hour/2; // 호수를 뿌린 후 대기시간을 설정하기 위한 변수
...
void setup() { // 무한 루프 문을 돌기 전 미리 설정하는 코드
  ...
  Serial.begin(9600); // 시리얼 모니터 사용(코드를 작성하는 도중 오류를 확인하기 위함)
}

void loop() { // 무한 루프 문
  if (ppap < hour/2) { // 호수를 뿌린 후, 대기 시간이 아직 지나지 않은 경우
  ...
  if (analogRead(sensor) >= 900 && ppap >= hour/2) { // 토양 수분 센서 값이 200보다 적은 경우
  ...
  if (pum) { // 펌프 상태가 작동인 경우
    if (buttont) { // 버튼을 누른 경우 (나눠서 분사)
      wait = true; // 나눠서 분사 시킨다
      count = 0; // 분사한 횟수를 0으로 설정한다
      pum = false; // 펌프 상태를 종료로 변경
    } else { // 아닌 경우 (한번에 분사)
      if (millis()-ptt <= 30000) { // 분사를 시작하고 30초가 지나지 않은 경우
        digitalWrite(IN1, HIGH); // 정방향 모터 작동
        digitalWrite(IN2, LOW); // 역방향 모터 비작동
      } else { // 분사를 시작하고 30초가 지난 경우
        pum = false; // 펌프 상태를 종료로 변경
  ...
  if (wait) { // 나눠서 분사시키는 상태인 경우
    if (millis()-ptt <= 6000) { // 분사를 시작하고 6초가 지나지 않은 경우
        digitalWrite(IN1, HIGH); // 정방향 모터 작동
        digitalWrite(IN2, LOW); // 역방향 모터 비작동
    } else if (che != true) { // 분사를 시작하고 6초가 지났으며, 대기 상태가 아닌 경우
        digitalWrite(IN1, LOW); // 정방향 모터 비작동
        digitalWrite(IN2, LOW); // 역방향 모터 비작동
        count++; // 분사 횟수에 1 추가
        pt = millis(); // 대기 시간 설정
        che = true; // 대기 상태로 설정
        if (count >= 5) { // 만약 분사를 5번 이상 한 경우
          wait = false; // 분사 상태 종료
          che = false; // 대기 상태 종료
          pt = 0; // 대기 시간 초기화
        }
